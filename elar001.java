...
public void ifContadorMethod(int contador_borrar, double default_value,float value) {
	if (contador_borrar==0) { //despois elmininar este if agora imos so facer probas
	     contador_borrar++;
	     
	     println("vamos coa regras");
	     
	     ifMsqlMethod( default_value, value);
	     
	     
}
}
	
	public void ifMsqlMethod(double default_value,float value){
		if ( msql.connect() ){
	        //CONFIGURATION
	        msql.query( "SELECT * FROM rules" );
	        
	        whileMethod(default_value, value);
	...
	public void whileMethod(double default_value,float value) {
		 while (msql.next()) {
	          ...
	          println("name: "+msql.getString("description"));
	           ...
	           while2Method(stop, default_value, value);
	           
	        }
	}

	public void while2Method(int stop, double default_value, float value) {
		 while (msql.next()) {
             ...
             ifMethod(stop , default_value);
             
             switchMethod(operator, stop, default_value, value);
	...
	public void ifMethod(int stop,double default_value) {
		if (msql.next()) {
	...
	public void switchMethod(int operator, int stop, double default_value, float value) {
		 switch (operator) {
         case 0:
            println("Maior");
            
            if2Method(stop, default_value, value);
	...
	public void if2Method(int stop, double default_value, float value) {
...
class Rule {
   ...
   Rule(int contador_borrar, double default_value,float value) {
   
	   
   ifContadorMethod(contador_borrar, default_value, value);
